...
def get_img_267bands(path):
    img = rasterio.open(path).read((2,6,7)).transpose((1, 2, 0))    
    img = np.float32(img)/MAX_PIXEL_VALUE
...
def generator_from_lists(images_path, masks_path, batch_size=32, shuffle = True, random_state=None, image_mode='2bands'):
...
EPOCHS = 70 # 훈련 epoch 지정
BATCH_SIZE = 32  # batch size 지정
IMAGE_SIZE = (256, 256) # 이미지 크기 지정
MODEL_NAME = 'band2' # 모델 이름
...
# 가중치 저장 위치
OUTPUT_DIR = 'D:\\_data\\dataset\\train_output\\'
...
# 중간 가중치 저장 이름
CHECKPOINT_PERIOD = 1
CHECKPOINT_MODEL_NAME = 'checkpoint-{}-{}-epoch_{{epoch:02d}}_band2.hdf5'.format(MODEL_NAME, save_name)
 
# 최종 가중치 저장 이름
FINAL_WEIGHTS_OUTPUT = 'model_{}_{}_band2.h5'.format(MODEL_NAME, save_name)
...
train_generator = generator_from_lists(images_train, masks_train, batch_size=BATCH_SIZE, random_state=RANDOM_STATE, image_mode="267")
validation_generator = generator_from_lists(images_validation, masks_validation, batch_size=BATCH_SIZE, random_state=RANDOM_STATE, image_mode="267")
...
# model.load_weights('D:\\_data\\dataset\\train_output\\checkpoint-band2-band2-epoch_19_band2.hdf5')

y_pred_dict = {}

for i in test_meta['test_img']:
    img = get_img_267bands(f'D:\\_data\\dataset\\test_img\\{i}')
...
dt = datetime.now()
joblib.dump(y_pred_dict, f'D:\\_data\\dataset\\train_output\\y_pred_{dt.day}_{dt.hour}_{dt.minute}.pkl')
