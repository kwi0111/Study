...
x_train = np.load(np_path + 'keras42_8_x_train.npy')
x_test = np.load(np_path + 'keras42_8_x_test.npy')
y_train = np.load(np_path + 'keras42_8_y_train.npy')
y_test = np.load(np_path + 'keras42_8_y_test.npy')

#2. 모델
model = Sequential()
model.add(Conv2D(64, (2,2), padding='same', strides=2, input_shape = (200,200,3), activation='relu'))
