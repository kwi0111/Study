...
#2. 모델 구성
model = Sequential()
model.add(Dense(10, input_dim = 30))        # sigmoid : 모든값 0~1로 한정 // 회귀 모델은 보통 리니어 // 2진분류는 시그모이드에 바이너리 크로스엔트로피
model.add(Dense(40))
model.add(Dense(80))
model.add(Dense(30))
model.add(Dense(10))
...
model.compile(loss='binary_crossentropy', optimizer='adam',      # loss='binary_crossentropy' -> 이진 분류때 씀 (sigmoid) // 실제값과 예측값의 차이
