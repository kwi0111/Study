...
class BaseModel(nn.Module):
    def __init__(self, num_classes):
        ...
        # EfficientNet-B1의 classifier를 제거합니다.
        self.backbone.classifier = nn.Identity()

        # EfficientNet-B1의 출력 특징 크기 확인 (여기서는 1280으로 가정)
        num_features = 1280  # 이 값은 모델에 따라 다를 수 있으니 확인 필요
        self.classifier = nn.Linear(num_features, num_classes)

    def forward(self, x):
        x = self.backbone(x)
        print("Shape after backbone:", x.shape)  # 디버깅을 위한 출력
        # 평균 풀링과 평탄화
        x = x.view(x.size(0), -1)
        x = self.classifier(x)
        return x
...
num_classes = len(le.classes_)  # 레이블 인코더를 사용하여 클래스 수를 동적으로 가져옵니다.
