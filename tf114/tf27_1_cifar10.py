...
# Layer1
w1 = tf.compat.v1.get_variable('w1', shape=[2,2,3,64], initializer=tf.contrib.layers.xavier_initializer())  # 커널사이즈 (2,2) / 컬러(채널 1개, 흑백) / 128개 필터(아웃풋) /
...
# Flatten
L_flat = tf.reshape(L3, [-1, 7*7*16])
print("플래튼 : ", L_flat)  # 플래튼 :  Tensor("Reshape:0", shape=(?, 1152), dtype=float32)

# Layer4 DNN
w4 = tf.compat.v1.get_variable('w4', shape=[7*7*16, 32], initializer=tf.contrib.layers.xavier_initializer()) 
